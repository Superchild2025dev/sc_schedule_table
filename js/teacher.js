...
function getBranchInfo(){
  ...
  if(selected==='yongam') return {id:'yongam', name:'용암점', fbPath:'schedule_yongam', aligoBranch:'용암점'};
  if(selected==='gagyeong') return {id:'gagyeong', name:'가경점', fbPath:'schedule', aligoBranch:'가경동'};
  return null;
}
...
function updateTeacherKeysTx(keys,mutator){
  ...
  const makeCtx=root=>({
    get(key,fallback){
      ...
      return parseStoredJSON(key,root[key],fallback);
    },
    set(key,val){
      if(!txKeySet.has(key)) throw new Error('트랜잭션 키 누락: '+key);
