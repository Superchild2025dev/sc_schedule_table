(function(){
  ...
  const STORAGE_KEY='sc_customer_voice_recent';
  const STATUS_ORDER=['received','acknowledged','investigating','resolved','answered'];
  let voiceFn=null;
  ...
  function updateMode(){
    ...
    $('submit-button').textContent=reply?'회원 확인 후 접수하기':'익명으로 접수하기';
  }
  function applyQuery(){
    ...
    $('back-link').href=`/parent.html${branch==='yongam'||branch==='gagyeong'?`?branch=${branch}`:''}`;
  }
  function payload(){
    const reply=selected('mode')==='reply';
    return {
  ...
  function saveReceipt(receipt){
    ...
    try{localStorage.setItem(STORAGE_KEY,JSON.stringify(receipt));}catch(e){}
  }
  function readReceipt(){
    try{return JSON.parse(localStorage.getItem(STORAGE_KEY)||'null');}catch(e){return null;}
  }
  function renderStatus(data){
    const current=Math.max(0,STATUS_ORDER.indexOf(data.status));
    $('status-ticket-number').textContent=data.ticketNumber||currentReceipt?.ticketNumber||'-';
    ...
    const message=String(data.publicReply||'').trim();
  ...
  async function refreshStatus(showFailure){
    currentReceipt=currentReceipt||readReceipt();
    if(!currentReceipt||!currentReceipt.ticketId||!currentReceipt.lookupToken) return;
    ...
    try{
      ...
      const response=await callable()({
        action:'status',
        branch:currentReceipt.branch,
        ticketId:currentReceipt.ticketId,
        lookupToken:currentReceipt.lookupToken
      });
      renderStatus(response.data||{});
    }catch(error){
      if(showFailure) toast(functionError(error));
  ...
  function bind(){
    ...
    $('check-status-button').addEventListener('click',()=>refreshStatus(true));
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    if(currentReceipt) refreshStatus(false);
  });
})();
