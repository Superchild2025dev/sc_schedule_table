...
function fixture(mode,overrides={}){
  const calls={prepared:[],loads:[],attendanceWrites:0,guestWrites:0,released:[]};
  let maps=plain(overrides.maps||{
    attendance:{'4시/화/1/1/2026-08-04':{s:'present'}},
    guests:{'4시/화/1/2026-08-04':[{gid:'old'}]},
  });
  const gateway={
    async ready(){ return {mode,generationId:mode==='v1'?'':'gen_1',valid:true}; },
    mode(){ return mode; },
  ...
  return {runtime,calls,maps:()=>plain(maps)};
}
