...
test('automatic expiration of a retirement reservation is not a second lower record', () => {
  assert.equal(
    context._scheduleAuditIsAutoRetireProcessingItem({
      _source:'audit',
      label:'자동 등록·제외 처리',
...
test('legacy and current automatic retirement audit formats are both recognized', () => {
  assert.equal(
    context._scheduleAuditIsAutoRetireProcessingItem({
      _source:'audit',
      label:'자동 등록·제외 처리',
